...
export class AppComponent {
  ...
  private mapLayer: TileLayer = tileLayer(
    "https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoieGVvbm9zIiwiYSI6ImNqanEzOGMzODV5aWszcG8zZ2NuYWRwY2MifQ.r0EfxOqx3A4CgXOIeKBwuQ",
  ...
  ngOnInit() {
    this.uploadControl.onAdd = (mapbox: Map) => {
      let container: HTMLElement = DomUtil.create(
        "label",
        "leaflet-bar leaflet-control leaflet-upload-btn"
      );
    ...
    this.infoControl.onAdd = (map: Map) => {
      let container: HTMLElement = DomUtil.create(
        "div",
        "leaflet-bar leaflet-control leaflet-info-panel"
      );
      container.setAttribute("_ngcontent-c0", "");
    ...
    if (files && files.length) {
      ...
      fileReader.onload = (fileLoadedEvent: any) => {
        ...
        let looper = setInterval(() => {
          if (c == 0) {
            this.uploadControl.getContainer().style.backgroundImage =
              "url(/assets/loading.gif)";
            this.uploadControl.getContainer().style.backgroundColor =
              "transparent";
          ...
          if (c < csvLines.length) {
            for (let i: number = c; i < csvLines.length && i - c < 500; i++) {
              ...
              if (csvLine.replace(/\s/g, "") != "") {
                ...
                if (prevCoords.longitude != undefined) {
                  ...
                  this.current_map.options.center = new LatLng(
                    coords.latitude,
                    coords.longitude
                  );
                }
            ...
            this.infoControl.getContainer().innerHTML =
              '<table style="width: 100%;padding: 4px;"><tr><td style="font-weight: bold;">Signal Strength</td><td style="font-weight: bold;">Network Type</td></tr><tr><td style="background-color:red;text-align:center;color:white">' +
              Math.floor((statistics.signal.r / statistics.total) * 100) +
              '%</td><td style="background-color:red;text-align:center;color:white">' +
              Math.floor((statistics.net.r / statistics.total) * 100) +
              '%</td></tr><tr><td style="background-color:yellow;text-align:center;">' +
              Math.floor((statistics.signal.y / statistics.total) * 100) +
              '%</td><td style="background-color:yellow;text-align:center;">' +
              Math.floor((statistics.net.y / statistics.total) * 100) +
              '%</td></tr><tr><td style="background-color:green;text-align:center;color:white">' +
              Math.floor((statistics.signal.g / statistics.total) * 100) +
              '%</td><td style="background-color:orange;text-align:center;color:white">' +
              Math.floor((statistics.net.o / statistics.total) * 100) +
              '%</td></tr><tr><td></td><td style="background-color:green;text-align:center;color:white">' +
              Math.floor((statistics.net.g / statistics.total) * 100) +
              "%</td></tr></table>";
